...
const AdminDashboard = () => {
  ...
  const toggleUser = (id) => {
    // If the same user is clicked again, close it. Otherwise, open the new one.
  ...
  return (
    <div className="flex min-h-screen bg-slate-50 text-slate-800 font-sans">
      ...
      {activeView === "users" && (
        <div className="flex-1 flex flex-col min-w-0 overflow-scroll relative p-4">
          {displayList.map((user) => (
            <div
              key={user.id}
              className="mb-4 border-2 border-gray-200 rounded-2xl overflow-hidden transition-all duration-300"
            >
              {/* User Card Header (The Clickable Part) */}
              <div
                onClick={() => toggleUser(user.id)}
                className={`p-4 cursor-pointer flex justify-between items-center hover:bg-indigo-50 transition-colors ${expandedUserId === user.id ? 'bg-indigo-100' : 'bg-white'
                  }`}
              >
                <div className="flex items-center gap-3">
                  <h2 className="font-extrabold text-white">
                    <span className="bg-indigo-950 px-4 py-2 rounded-xl">User: {user.name}</span>
                  </h2>
                  <span className="text-sm text-gray-500 font-medium">
                    ({user.list.length} Complaints)
                  </span>
                </div>

                {/* Visual Indicator (Chevron) */}
                <span className={`transform transition-transform duration-200 ${expandedUserId === user.id ? 'rotate-180' : ''}`}>
                  ▼
                </span>
              </div>

              {/* Complaints Table (Conditional Content) */}
                    ...
                    <tbody>
                      {user.list.map((item) => (
                        <tr key={item.id} className="border-b border-gray-50 hover:bg-gray-50">
                          ...
                          <td className="p-2">
                            <span className={`badge-${item.status.toLowerCase()} px-2 py-1 rounded-full text-xs`}>
                              {item.status}
                            </span>
                          </td>
                        </tr>
